...
export default function EventDetailCard(props) {
    ...
    const [eventVariable, setEventVariable] = React.useState({eventVariableMap:{}, title:""});
    ...
    const createPersonalEvent = () => {
      setIsLoading(true);
      API.fetchEventVariable(event.id).then(response=>{
    ...
    const create = () => {
      ...
      Object.keys(eventVariable.eventVariableMap).forEach((key)=>{
        eventVariable.eventVariableMap[key].value = document.getElementById(`variable-${key}`).value
      })
      API.newPersonalEvent({eventId:event.id, eventVariableMap: eventVariable.eventVariableMap}).then(
        response => {
    ...
    const createTitle = (data) => {
      Object.keys(data?.eventVariableMap).forEach((key)=>{
        data.title = data?.title.replace(`$${key}$`, (
        `<select id="variable-${key}">${data.eventVariableMap[key].options.map((value)=>`<option value="${value}" ${value === data.eventVariableMap[key].value?"selected":""}>${value}</option>`).join(',')}</select>`
        ))
      })
            ...
            !editMode?(
              <>
                ...
                <div  style={{position:"absolute", right:"20px"}}>
                  ...
                  <Button size="mid" onClick={()=>{setEditMode(false)}} color="info">
                    Cancel
                  </Button>
                </div>
            )
